...
//        btnAnimSet.startAnimation(setAnimation)
         ...
         *
         * 属性动画进行、结束时，他的响应区域是跟着视图动了，但是layout还是在原地，getTop()自始至终值都没变
         * 如果结束后也设置layout的话，视图会相对移动
         *
         * 属性动画在完成后是变化了视图的相对位置，就是说一个点在（0，0），用属性动画后，向下移动到了（0，100），
         * 这时它的视图的layout位置还是在（0，0），只是显示在（0，100）上。
         * 若这是改变它的layout，用View.layout(int l, int t, int r, int b)方法改变到点（0,100）上，这时视图显示却是在（0，200）上。
         */
    }
